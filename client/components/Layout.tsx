...
export function Layout({ children }: LayoutProps) {
  ...
  const [expandedGroups, setExpandedGroups] = useState<string[]>(["core", "clinical"]);
    ...
    return [
      ...
      },
      {
        ...
        items: [
          ...
          { name: "Patient Management", href: "/ehr/intake", icon: Users },
          { name: "Clinical Tools", href: "/ehr/clinical-tools", icon: Stethoscope },
          { name: "Practice Management", href: "/ehr/scheduling", icon: Building },
          { name: "Patient Engagement", href: "/ehr/telehealth", icon: HeartPulse },
        ]
      },
      {
        id: "ecommerce",
        ...
        items: [
          ...
          { name: "E-commerce Analytics", href: "/analytics/ecommerce", icon: BarChart3 },
        ]
      },
      {
        id: "business",
    ...
    switch (user.role) {
      ...
      case "doctor":
        return [
          ...
          { name: "EHR Overview", href: "/ehr", icon: Cloud },
          { name: "Patient Intake", href: "/ehr/intake", icon: UserPlus },
          { name: "AI Medical Scribe", href: "/ehr/ai-scribe", icon: Brain },
          { name: "Programs", href: "/ehr/programs", icon: HeartPulse },
          { name: "Multi-Provider", href: "/ehr/providers", icon: Users },
          { name: "Patient Portal", href: "/ehr/portal", icon: Globe },
          { name: "Patient Analytics", href: "/analytics", icon: BarChart3 },
          { name: "AI Chat", href: "/chat", icon: MessageCircle },
        ];
      case "pharmacist":
        return [
  ...
  const isActiveRoute = (href: string) => {
    ...
    if (href === "/ehr/intake" && location.pathname.startsWith("/ehr/")) {
      ...
      return ["/ehr/intake", "/ehr/charting", "/ehr/care-plans"].includes(location.pathname);
    }
    if (href === "/ehr/clinical-tools" && location.pathname.startsWith("/ehr/")) {
      // Clinical Operations category routes
      return ["/ehr/clinical-tools", "/ehr/ai-scribe", "/ehr/providers"].includes(location.pathname);
    }
    if (href === "/ehr/scheduling" && location.pathname.startsWith("/ehr/")) {
      // Practice Management category routes
      return ["/ehr/scheduling", "/ehr/billing", "/ehr/insurance"].includes(location.pathname);
    }
    if (href === "/ehr/reporting") {
      // Analytics & Reporting category routes
      return ["/ehr/reporting", "/analytics", "/ehr/workflows", "/ehr/quality"].includes(location.pathname);
    }
    if (href === "/ehr/telehealth" && location.pathname.startsWith("/ehr/")) {
      // Patient Engagement category routes
      return ["/ehr/telehealth", "/ehr/messaging", "/ehr/programs", "/ehr/journaling", "/ehr/portal"].includes(location.pathname);
    }
    ...
    <div className="min-h-screen aurora-bg flex">
      ...
      <aside className="hidden lg:flex lg:flex-col lg:w-64 lg:fixed lg:inset-y-0 glass-morphism lg:backdrop-blur-xl lg:border-r lg:border-border/50 lg:shadow-2xl">
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          {user?.role === "admin" ? (
            // Admin navigation with collapsible groups
            navigation.map((group: any) => {
              const GroupIcon = group.icon;
              const isExpanded = expandedGroups.includes(group.id);
              const hasActiveItem = group.items?.some((item: any) => isActiveRoute(item.href));
      ...
      <div className="flex-1 lg:ml-64">
                    ...
                    {user?.role === "admin"
                      ? (() => {
                          ...
                          for (const group of navigation) {
                            const foundItem = group.items?.find((item: any) => item.href === location.pathname);
                            if (foundItem) return foundItem.name;
                          }
                          return "Admin Dashboard";
                        })()
                      : navigation.find((nav: any) => nav.href === location.pathname)?.name || "Home"
                    }
                  </h1>
                  {isAuthenticated && user && (
        ...
        {/* EHR Sub-Navigation */}
        {(user?.role === "admin" || user?.role === "doctor") && <EHRSubNavigation />}
